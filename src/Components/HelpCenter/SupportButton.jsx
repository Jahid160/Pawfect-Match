      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <div className="flex justify-between items-center bg-orange-500 p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="text-white">
                  ...
                  <p className="opacity-90 text-[11px]">● Online • 24/7 support</p>
                </div>
              </div>
            ...
            {/* Chat Body */}
            <div ref={scrollRef} className="flex flex-col gap-4 bg-gray-50/50 p-4 h-[400px] overflow-y-auto">
              {messages.map((msg) => (
                <div key={msg.id} className={`flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                  <div className={`max-w-[85%] p-4 rounded-2xl shadow-sm text-sm leading-relaxed ${msg.sender === 'user'
                    ...
                    {msg.sender === 'user' && <div className="opacity-70 mt-1 text-[10px] text-right">{msg.time}</div>}
                  </div>
                  {msg.sender === 'bot' && <span className="mt-1 ml-1 text-[10px] text-gray-400">{msg.time}</span>}
                </div>
              ))}
            </div>
            ...
            <div className="bg-white p-4 border-gray-100 border-t">
              <form onSubmit={handleSendMessage} className="relative flex items-center gap-2">
                ...
                />
                <button type="submit" className={`shrink-0 w-12 h-12 ${!inputValue ? "bg-orange-500" : "bg-orange-600"} rounded-2xl flex items-center justify-center text-white shadow-md hover:opacity-90 transition-opacity`}>
