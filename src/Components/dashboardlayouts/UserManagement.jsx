  ...
  return (
    <div className="bg-base-200 p-4 lg:p-10 min-h-screen font-sans text-neutral">
      ...
      <div className="flex md:flex-row flex-col justify-between items-start md:items-center gap-6 mb-12">
        <h1 className="font-black text-neutral text-5xl tracking-tight">
          User{" "}
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-10">
        <div className="md:col-span-8 relative">
          <Search
            className="top-1/2 left-5 absolute text-neutral/30 -translate-y-1/2"
            size={20}
          />
          <input
            type="text"
                  ...
                  >
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <div className="flex justify-center items-center bg-primary/10 rounded-2xl w-10 h-10 font-black text-primary text-sm">
                          {u.name?.charAt(0)}
                        </div>
                        <div>
                    ...
                    <td className="px-8 py-6">
                      <span
                        className={`px-3 py-1 rounded-xl text-[10px] font-black uppercase ${u.status === "active" || u.status !== "block" ? "bg-success/10 text-success" : "bg-error/10 text-error"}`}
                      >
                        {u.status || "active"}
                      </span>
                    </td>
                    <td className="px-8 py-6 text-neutral/40 font-bold text-xs">
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <button
                          ...
                          className="p-2.5 bg-base-200 text-neutral/40 hover:text-primary rounded-xl transition-all"
                        >
                          <Eye size={18} />
                        </button>
                        <button
                          ...
                          className="p-2.5 bg-base-200 text-neutral/40 hover:text-success rounded-xl disabled:opacity-20"
                        >
                          <UserCheck size={18} />
                        </button>
                        <button
                          onClick={() => handleAction(u._id, blockUser)}
                          disabled={u.status === "block"}
                          className="p-2.5 bg-base-200 text-neutral/40 hover:text-error rounded-xl disabled:opacity-20"
                        >
                          <UserX size={18} />
                        </button>
                      </div>
