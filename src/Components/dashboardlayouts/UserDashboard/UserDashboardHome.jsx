...
import { 
  PawPrint, Heart, Clock, MessageCircle, 
...
const UserDashboardHome = () => {
  ...
  const overviewCards = [
    { title: "Adopted Pets", value: 2, icon: PawPrint, color: "bg-orange-500" },
    { title: "Favorite Pets", value: 5, icon: Heart, color: "bg-rose-500" },
    { title: "Pending Requests", value: 1, icon: Clock, color: "bg-amber-500" },
    { title: "Messages", value: 3, icon: MessageCircle, color: "bg-blue-500" },
  ];

  const recentRequests = [
    { pet: "Golden Retriever", status: "Pending", date: "12 Mar 2026", color: "text-amber-600 bg-amber-50" },
    { pet: "Persian Cat", status: "Approved", date: "05 Mar 2026", color: "text-emerald-600 bg-emerald-50" },
  ];

  const recommendedPets = [
    { name: "Husky", breed: "Siberian", image: "https://images.unsplash.com/photo-1601758228041-f3b2795255f1" },
    { name: "Labrador", breed: "Retriever", image: "https://images.unsplash.com/photo-1558788353-f76d92427f16" },
    { name: "Persian Cat", breed: "Longhair", image: "https://images.unsplash.com/photo-1518791841217-8f162f1e1131" },
  ];
  ...
  return (
    <div className="p-6 lg:p-10 space-y-10 bg-[#FDFCFB] min-h-screen">
        ...
        {overviewCards.map((card, index) => (
          ...
          >
            <div>
              ...
              <h2 className="text-3xl font-black text-slate-800">{card.value}</h2>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8">
          <div className="flex justify-between items-center mb-6">
            ...
            <Link href="#" className="text-orange-500 text-sm font-bold flex items-center gap-1 hover:underline">
              ...
              <tbody className="divide-y divide-slate-50">
                {recentRequests.map((item, i) => (
                  <tr key={i} className="group transition-colors hover:bg-slate-50/50">
                    <td className="py-4 px-2 font-bold text-slate-700">{item.pet}</td>
                    <td className="py-4 px-2">
                      <span className={`px-3 py-1.5 text-[10px] font-black rounded-full uppercase tracking-tighter ${item.color}`}>
                        {item.status}
                      </span>
                    </td>
                    <td className="py-4 px-2 text-right text-slate-400 font-medium text-sm">{item.date}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="lg:col-span-5 bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8">
          ...
          <div className="grid grid-cols-1 gap-3">
            ...
            <button className="flex items-center justify-between w-full p-4 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-2xl font-bold transition-all">
              Update Profile <ExternalLink size={18} className="text-slate-400" />
            </button>
            <button className="flex items-center justify-between w-full p-4 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-2xl font-bold transition-all">
              Support Center <MessageCircle size={18} className="text-slate-400" />
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <h2 className="text-2xl font-black text-slate-800">New On The Block 🦴</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {recommendedPets.map((pet, index) => (
            <div key={index} className="group bg-white border border-slate-100 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500">
              ...
              <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                  <div>
                    <h3 className="font-black text-slate-800 text-lg">{pet.name}</h3>
                    <p className="text-slate-400 text-xs font-bold">{pet.breed}</p>
                  </div>
