...
const DoctorManagement = ({ allOrders = [] }) => {
  // ১. লজিক: শুধুমাত্র AdminAccepted স্ট্যাটাসের অর্ডারগুলো বের করা
  const pendingVaccinations = allOrders.filter(order => order.status === "AdminAccepted");
  const completedVaccinations = allOrders.filter(order => order.status === "DoctorAccepted" || order.isCompleted);
  ...
  return (
    <div className="bg-[#F8FAFC] p-6 lg:p-10 min-h-screen font-sans text-slate-900 pt-28">
      ...
      <div className="gap-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mb-12">
        ...
        <StatMiniCard label="Completed" value={completedVaccinations.length} icon={<CheckCircle2 size={20}/>} color="bg-orange-50 text-orange-600" />
        
        {/* ২. রিয়েল টাইম পেন্ডিং ভ্যাকসিনেশন কাউন্ট */}
        <StatMiniCard 
          label="Pending Vaccinations" 
          value={pendingVaccinations.length < 10 ? `0${pendingVaccinations.length}` : pendingVaccinations.length} 
      ...
      <div className="gap-8 grid grid-cols-1 lg:grid-cols-2">
        {doctors.map((doctor, i) => {
          // ৩. চেক করা হচ্ছে এই ডক্টর কি ভ্যাকসিনেশন স্পেশালিস্ট কি না
            ...
            >
              ...
              <div className="z-10 relative flex sm:flex-row flex-col items-start gap-6">
                ...
                <div className="flex-1">
                  ...
                  {/* ৪. ডক্টর কার্ডের ভেতরে পেন্ডিং টাস্ক লিস্ট দেখানো */}
                  {isVaccinator && pendingVaccinations.length > 0 && (
                    <div className="mt-6 pt-6 border-t border-dashed border-slate-100">
                      <div className="flex justify-between items-center mb-3">
                        <p className="font-black text-[10px] text-purple-600 uppercase tracking-widest flex items-center gap-2">
                          <Syringe size={12}/> Pending Tasks ({pendingVaccinations.length})
                        </p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {pendingVaccinations.slice(0, 3).map(order => (
                          <div key={order._id} className="bg-purple-50 px-3 py-1.5 rounded-xl border border-purple-100 flex items-center gap-2">
                             <span className="w-1.5 h-1.5 bg-purple-500 rounded-full animate-pulse"></span>
                             <p className="text-[10px] font-black text-purple-700">{order.vaccineName}</p>
                          </div>
                        ))}
                        {pendingVaccinations.length > 3 && (
                          <span className="text-[10px] text-slate-400 font-bold self-center">+{pendingVaccinations.length - 3} more</span>
                        )}
                      </div>
                    </div>
                  )}
