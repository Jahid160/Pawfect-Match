...
const heroSlides = [
  {
    ...
    headline: 'Every Pet Deserves a <span class="text-orange-500 decoration-orange-200 italic">Pawfect</span> Match.',
          ...
          <SwiperSlide key={slide.id}>
            <div className="flex lg:flex-row flex-col items-center bg-transparent mx-auto px-6 lg:px-12 w-full max-w-7xl h-full">
              ...
              <div className="z-20 flex flex-col flex-1 justify-center py-10 lg:py-0 text-start">
                <div className="inline-flex items-center gap-3 bg-orange-50 mb-8 px-5 py-2.5 border border-orange-100 rounded-full w-fit font-black text-[10px] text-orange-600 uppercase tracking-[0.3em]">
                ...
                <h1 
                  className="mb-8 font-black text-slate-900 lg:text-[85px] text-5xl md:text-7xl leading-[0.95] tracking-[-0.04em]"
                  dangerouslySetInnerHTML={{ __html: slide.headline }}
                />
                
                <p className="mb-12 max-w-xl font-medium text-slate-500 text-lg md:text-xl leading-relaxed">
                  {slide.description}
                </p>
                ...
                <div className="flex sm:flex-row flex-col gap-5">
                  <Link
                    href="/all-pets"
                    className="group flex justify-center items-center gap-3 bg-slate-900 hover:bg-orange-500 shadow-slate-200 shadow-xl hover:shadow-orange-200 px-12 py-5 rounded-2xl font-black text-[11px] text-white uppercase tracking-[0.2em] transition-all hover:-translate-y-1 duration-500 transform"
                  >
                    {slide.buttonText}
                  ...
                  <Link
                    href="/contact"
                    className="hover:bg-orange-50 px-12 py-5 border-2 border-slate-100 hover:border-orange-500 rounded-2xl font-black text-[11px] text-slate-900 hover:text-orange-600 text-center uppercase tracking-[0.2em] transition-all duration-300"
                  >
                    Get in Touch
                  </Link>
                </div>
              ...
              <div className="relative flex flex-1 justify-center items-center pb-24 lg:pb-0 w-full h-[500px] lg:h-[700px]">
                <div className="group relative w-full h-[400px] lg:h-[600px]">
                    ...
                    <div className="top-10 -right-4 lg:-right-8 z-30 absolute flex items-center gap-3 bg-white/95 shadow-xl px-6 py-4 border border-slate-50 rounded-[2.2rem] animate-float">
                        <div className="bg-orange-100 p-2.5 rounded-xl text-lg">✨</div>
                        <div>
                            <p className="font-black text-slate-900 text-sm leading-none tracking-tight">{slide.stats}</p>
                            <p className="mt-1.5 font-bold text-[9px] text-slate-400 uppercase tracking-[0.1em]">Happy Homes</p>
                        </div>
                    </div>

                    {/* Floating Info Card 2 */}
                    <div className="bottom-12 -left-6 lg:-left-10 z-30 absolute flex items-center gap-4 bg-white shadow-2xl p-6 border border-slate-50 rounded-[2.5rem] animate-float-delayed">
                        <div className="bg-slate-900 shadow-lg shadow-slate-200 p-3.5 rounded-2xl text-white text-xl">
                            {slide.icon}
                        </div>
                        <div>
                            <p className="font-black text-slate-900 text-base leading-none tracking-tight">Pet Verified</p>
                            <p className="mt-1.5 font-bold text-[10px] text-orange-500 uppercase tracking-[0.2em]">100% Healthy</p>
                        </div>
                    </div>
                </div>
              </div>
