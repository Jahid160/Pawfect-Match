...
import { FaLinkedinIn, FaTwitter, FaEnvelope, FaSearch, FaUserAlt, FaStethoscope, FaAward, FaUsers, FaHeart, FaChevronRight } from 'react-icons/fa';
    ...
    <div className="bg-white selection:bg-orange-100 min-h-screen">
          ...
          <div className="flex lg:flex-row flex-col justify-between items-center gap-16">
            ...
            >
              ...
              <h1 className="mb-8 font-black text-gray-900 text-6xl lg:text-8xl leading-[0.95] tracking-[-0.04em]">
                Meet Our <br /> <span className="text-orange-500 italic">Certified</span> Experts
              </h1>
              <p className="mb-10 max-w-xl font-medium text-gray-500 text-xl leading-relaxed">
                Our team of dedicated professionals brings decades of collective experience in veterinary medicine to ensure your pets live their best lives.
              </p>
              ...
              <div className="group relative max-w-lg">
                ...
                />
                <FaSearch className="top-1/2 left-7 absolute text-gray-400 group-focus-within:text-orange-500 transition-colors -translate-y-1/2" size={20} />
                <button className="top-2 right-2 absolute bg-gray-900 hover:bg-orange-600 px-8 py-4 rounded-[1.8rem] font-black text-[10px] text-white uppercase tracking-widest active:scale-95 transition-all">
                  Search
                </button>
              </div>
            </motion.div>
            ...
            >
              {[
                ...
                <div key={i} className="group bg-white hover:bg-orange-600 shadow-orange-100/20 shadow-xl p-8 border border-gray-100 rounded-[2.5rem] text-center transition-all duration-500 cursor-default">
                  ...
                  <h4 className="font-black text-gray-900 group-hover:text-white text-3xl tracking-tighter transition-colors">{stat.val}</h4>
                  <p className="font-bold text-[10px] text-gray-400 group-hover:text-orange-100 uppercase tracking-[0.1em] transition-colors">{stat.label}</p>
                </div>
              ))}
            </motion.div>
          </div>
      ...
      <section className="mx-auto px-4 sm:px-8 py-32 max-w-7xl">
        <div className="flex justify-between items-end mb-20">
          <div>
            <span className="font-black text-orange-500 text-xs uppercase tracking-widest">Our Directory</span>
            <h2 className="mt-2 font-black text-gray-900 text-4xl md:text-5xl tracking-tighter">VERIFIED SPECIALISTS</h2>
          </div>
          <div className="hidden md:block font-bold text-gray-400 text-sm italic uppercase tracking-widest">
            {allExperts.length} Curated Profiles
          </div>
        </div>
            ...
            >
              <div className="relative shadow-2xl shadow-orange-100/50 rounded-[3rem] w-full h-[420px] overflow-hidden">
                <Image
                  ...
                  alt={expert.name}
                  fill
                  className="grayscale-0 object-cover group-hover:scale-110 transition-transform duration-1000 ease-out"
                />
                ...
                <div className="absolute inset-0 flex justify-center items-center bg-gray-900/60 opacity-0 group-hover:opacity-100 backdrop-blur-sm transition-all duration-500">
                  <div className="flex gap-4 transition-transform translate-y-10 group-hover:translate-y-0 duration-500">
                    <button className="bg-white hover:bg-orange-500 shadow-xl p-4 rounded-full text-gray-900 hover:text-white transition-all"><FaLinkedinIn size={20} /></button>
                    <button className="bg-white hover:bg-orange-500 shadow-xl p-4 rounded-full text-gray-900 hover:text-white transition-all"><FaTwitter size={20} /></button>
                    <button className="bg-white hover:bg-orange-500 shadow-xl p-4 rounded-full text-gray-900 hover:text-white transition-all"><FaEnvelope size={20} /></button>
                  </div>
                </div>
              ...
              <div className="mt-8 text-center">
                <h3 className="mb-1 font-black text-gray-900 text-2xl leading-none tracking-tight transition-colors">{expert.name}</h3>
                <p className="mb-2 font-black text-[10px] text-orange-500 uppercase tracking-[0.2em]">{expert.role}</p>
                <p className="mb-8 font-medium text-gray-400 text-xs italic tracking-wide">Specalized in {expert.speciality}</p>
        ...
        <div className="group relative bg-slate-900 p-16 md:p-24 rounded-[4rem] overflow-hidden">
          <div className="z-10 relative flex lg:flex-row flex-col justify-between items-center gap-12">
            <div className="max-w-xl lg:text-left text-center">
              <h2 className="mb-6 font-black text-white text-5xl md:text-7xl leading-[1] tracking-tighter">Are you a <span className="text-orange-500">Pet Expert?</span></h2>
              <p className="font-medium text-slate-400 text-xl leading-relaxed">Join our elite network of veterinary professionals and help us build the future of pet welfare.</p>
            </div>
