...
const NewsletterGame = () => {
    ...
    if (status === "unauthenticated") {
      ...
      Swal.fire({
        ...
        customClass: {
          ...
          confirmButton: 'rounded-full px-10 py-3 font-bold uppercase tracking-widest'
        },
        buttonsStyling: true,
        showClass: {
  ...
  return (
    <section className="bg-[#FDFCFB] py-24 overflow-hidden font-sans text-center">
        ...
        <div className="relative bg-gradient-to-br from-orange-500 via-orange-550 to-orange-600 shadow-[0_50px_100px_-30px_rgba(249,115,22,0.5)] p-1 md:p-2 rounded-[3.5rem]">
          <div className="relative flex flex-col justify-center items-center bg-orange-500 px-8 md:px-20 py-16 md:py-24 border border-white/20 rounded-[3.2rem] min-h-[600px] overflow-hidden">
            ...
            <div className="z-10 w-full max-w-3xl">
              {score < 5 ? (
                <>
                  <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-3 bg-white/20 backdrop-blur-md mb-6 px-6 py-2 border border-white/10 rounded-full font-black text-[10px] text-white uppercase tracking-[0.2em]">
                  ...
                  <h2 className="mb-4 font-black text-white text-5xl md:text-7xl leading-none tracking-tighter">
                    Feed the <span className="text-orange-200">Hungry Pet!</span>
                  </h2>
                  <p className="opacity-90 mb-10 font-bold text-orange-50 text-xl">
                    Catch 5 Treats to Join the Pack: <span className="bg-white/20 px-4 py-1 rounded-full">{score} / 5</span>
                  </p>
                  ...
                  <div className="relative bg-white/5 backdrop-blur-sm border-2 border-white/20 border-dashed rounded-[3rem] w-full h-[400px] overflow-hidden cursor-crosshair">
                    ...
                    <motion.div animate={{ y: [0, -10, 0], scale: [1, 1.05, 1] }} transition={{ repeat: Infinity, duration: 3, ease: "easeInOut" }} className="bottom-6 left-1/2 absolute w-32 md:w-44 h-32 md:h-44 -translate-x-1/2">
                      <img src={PET_IMAGE} alt="Pet" className="drop-shadow-2xl w-full h-full object-contain" />
                    </motion.div>
                  </div>
                </>
              ) : (
                <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white/10 shadow-2xl backdrop-blur-2xl p-10 md:p-16 border border-white/30 rounded-[4rem]">
                  <div className="inline-block relative mb-8">
                    <FaTrophy className="drop-shadow-[0_0_30px_rgba(253,224,71,0.5)] text-yellow-300 text-8xl" />
                  </div>

                  <h2 className="mb-6 font-black text-white text-5xl md:text-6xl leading-none tracking-tight">You're a True Pet Lover!</h2>
                  ...
                  <div className="flex flex-col items-center gap-6">
                    <div className="flex sm:flex-row flex-col gap-4 w-full max-w-md">
                        <input 
                            type="email" 
                            readOnly 
                            value={status === "authenticated" ? session?.user?.email : ""} 
                        ...
                        >
                            {status === "authenticated" ? "Subscribe" : "Login to Join"} <FaPaperPlane />
                        </button>
                    </div>

                    <button onClick={() => {setScore(0); setGameActive(true)}} className="mt-4 font-bold text-[10px] text-white/70 hover:text-white underline uppercase tracking-[0.2em] transition-colors">
