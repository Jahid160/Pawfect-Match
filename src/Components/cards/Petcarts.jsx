    ...
    <div className="group relative flex flex-col bg-white shadow-md hover:shadow-2xl border border-gray-100 rounded-3xl overflow-hidden transition-all hover:-translate-y-2 duration-500">
      ...
      <div className="flex flex-col flex-1 p-5 sm:p-6">
        <div className="flex justify-between items-start gap-2 mb-3">
          ...
          <div
            className={`shrink-0 rounded-md px-2 py-0.5 text-[9px] sm:text-[10px] font-bold uppercase ${pet.status === "Available" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
          >
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 mb-5">
          ...
          <div className="flex items-center gap-1.5 text-[10px] text-gray-500 sm:text-xs">
            ...
            <span className="max-w-[80px] sm:max-w-[120px] font-semibold truncate">
        ...
        <div className="flex justify-between items-center mt-auto pt-4 border-gray-100 border-t">
          <Link
            href={`/all-pets/${petId}`}
            className="font-black text-[10px] text-gray-400 sm:text-[11px] group-hover:text-orange-600 uppercase tracking-[1.5px] sm:tracking-[2px] transition-all"
          >
            Meet {pet.name}
          </Link>
          <Link
            href={`/all-pets/${petId}`}
            className="flex justify-center items-center bg-gray-100 group-hover:bg-orange-500 rounded-xl w-9 sm:w-10 h-9 sm:h-10 text-gray-400 group-hover:text-white transition-all group-hover:translate-x-1 duration-300"
          >
...
const Petcarts = ({ pets = [] }) => {
  ...
  const filteredPets = useMemo(() => {
    ...
    if (selectedCategory !== "All") {
      result = result.filter(
        (p) =>
          (p.species || p.category)?.toLowerCase() ===
        ...
        <div className="flex md:flex-row flex-col justify-between items-start md:items-end gap-6 mx-auto mb-10 sm:mb-16 max-w-7xl">
          ...
          <div className="w-full max-w-md">
            <p className="text-gray-500 text-base sm:text-lg text-left md:text-right leading-relaxed">
              Every paw leaves a footprint. Browse our gallery of loving pets
        ...
        <div className="z-10 relative mx-auto max-w-7xl">
          <div className="flex justify-between items-center mb-6 px-2">
            <p className="font-bold text-[10px] text-gray-400 sm:text-xs uppercase tracking-widest">
              Found{" "}
              <span className="text-gray-900">{filteredPets.length} pets</span>
            </p>
          </div>
          ...
          <div className="gap-6 sm:gap-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {paginatedPets.length > 0 ? (
              ...
              <div className="col-span-full py-20 text-center">
                ...
                <h3 className="font-bold text-gray-400 text-xl tracking-tighter">
                  No furry friends found matching your search.
                </h3>
              </div>
            )}
          </div>
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-3 mx-auto mt-16">
            <button
              ...
              className="bg-white disabled:opacity-30 shadow-sm p-3 border border-gray-200 rounded-xl active:scale-90 transition-all"
            >
