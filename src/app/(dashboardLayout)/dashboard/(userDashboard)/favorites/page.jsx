...
} from "@/action/userServerDash/savePetsAction";
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-slate-500 font-medium">
              You have {pets.length} pets in your adoption wishlist.
            </p>
          </div>
          <Link
            href="/all-pets"
            className="group btn btn-primary px-8 rounded-full shadow-lg shadow-primary/20 transition-all hover:scale-105"
          >
            Explore More{" "}
            <ArrowRight
              size={18}
        ...
        {pets.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-[3rem] bg-white border border-slate-100 py-32 text-center shadow-xl shadow-slate-200/50">
            ...
            <p className="text-slate-400 mt-2 max-w-xs mx-auto">
              Looks like you haven't found your perfect furry friend yet.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {pets.map((pet) => (
              <div
                key={pet._id}
                className="group bg-white rounded-[2.5rem] border border-slate-100 shadow-sm transition-all duration-300 hover:shadow-2xl hover:shadow-slate-200 hover:-translate-y-2 overflow-hidden"
              >
                {/* Image Area */}
